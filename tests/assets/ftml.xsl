...
<html>
	<head>
		...
		<xsl:for-each select="/ftml/head/fontsrc">
			<xsl:if test="starts-with(.,'url(https://fonts.googleapis.com/css')">
				<link rel = "stylesheet" >
					<xsl:attribute name="href"><xsl:value-of select="substring-before(substring-after(., '(' ), ')' )"/></xsl:attribute>
				</link>
			</xsl:if>
		</xsl:for-each>
		<style>
	...
	<xsl:for-each select="/ftml/head/fontsrc">
		<xsl:choose>
			<xsl:when test="starts-with(.,'url(https://fonts.googleapis.com/css')">
				<!-- handle Google Font APIs -->
				<!-- extract first family name from GF url, stripping off (1) subsequent font names, (2) subsets, (3) weights & styles -->
				<!-- xsl:variable name="family" select="translate(substring-before(substring-before(substring-before(substring-after(.,'family='),'|'),'&amp;'),':'),'+',' ')"/ -->
				<xsl:variable name="family" select="translate(substring-before(substring-after(., 'family='), ')' ), '+', ' ')"/>
				.font<xsl:number/> {font-family: <xsl:value-of select="$family"/>}
			</xsl:when>
			<xsl:otherwise>
